...
public class addQuestion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        {
            session.setAttribute("category","java");
            boolean b = false;
            try{
               ...
               if(session.getAttribute("type").toString().equals("sample")) 
               {
            ...
            {
                RequestDispatcher rd = request.getRequestDispatcher("paperexists.html");
                rd.forward(request, response);
            }
        }
        else if(request.getParameter("python")!=null)
        {
            session.setAttribute("category","python");
            boolean b = false;
            try{
                Class.forName("com.mysql.jdbc.Driver");
                Connection con = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/presentation","root","");
